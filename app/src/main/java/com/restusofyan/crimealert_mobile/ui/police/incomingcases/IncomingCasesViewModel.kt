...
@HiltViewModel
class IncomingCasesViewModel @Inject constructor(
            ...
            try {
                ...
                if (response.isSuccessful) {
                    _reports.value = response.body()?.listReports?.filterNotNull()
                        ?.filter { it.statusKasus.equals(STATUS_BELUM_DITANGANI, ignoreCase = true) }
                } else {
    ...
    companion object {
        private const val STATUS_BELUM_DITANGANI = "belum_ditangani"
    }
}
