...
@HiltViewModel
class ValidateInsidensViewModel @Inject constructor(
    ...
    fun compressImage(filePath: String, maxSize: Long = 1 * 1024 * 1024): File? {
        ...
        if (bitmap == null) {
            Log.e("ValidateInsidensViewModel", "Failed to create bitmap")
            return null
        }
    ...
    fun validateInsiden(
        ...
        storyRepository.validateNewInsiden(
            token,
            titlePart,
            descriptionPart,
            latitudePart,
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful && response.body()?.error == false) {
                    _uploadResult.value =
                        UploadResult.Success("Incident validation report uploaded successfully")
                } else {
                    val errorMessage = response.body()?.message ?: "Failed to upload incident validation report"
                    _uploadResult.value = UploadResult.Error(errorMessage)
                }
            }
